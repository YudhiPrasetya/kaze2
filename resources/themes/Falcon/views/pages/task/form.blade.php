		...
		<x-bootstrap::column breakpoint="EXTRA_SMALL|12;MEDIUM|9">
			<x-bootstrap::card>
					...
					<x-bootstrap::row class="align-items-baseline justify-content-between">
						<x-bootstrap::column breakpoint="EXTRA_SMALL|6;SMALL|auto;MEDIUM|9" class="d-flex flex-column align-items-baseline">
							<h5 class="fs-2 font-weight-semi-bold mb-0 text-nunito py-2 py-xl-0 text-truncate w-100 text-truncate">
								@if(!$model->title)
									Task
									<small class="fs-0 text-muted d-block">Add new task</small>
								@else
									{{ $model->title }}
				...
				<x-bootstrap::card.body class="bg-light">
					<x-bootstrap::row>
						<x-bootstrap::column breakpoint="EXTRA_SMALL|6;SMALL|auto;MEDIUM|5">
							{!! form_row($form->employee_id, ['attr' => ['data-value' => $model->employee_id]]) !!}
							{!! form_row($form->priority_id, ['attr' => ['data-value' => $model->priority_id]]) !!}
							{!! form_row($form->dateline) !!}
						</x-bootstrap::column>
						<x-bootstrap::column>
							{!! form_row($form->title) !!}
							{!! form_row($form->description, ['attr' => ['class' => 'tinymce', 'rows' => 10]]) !!}
						</x-bootstrap::column>
					</x-bootstrap::row>
