 ...
 * @file   custom-scrollbar-touch.js
...
(function (factory) {
		...
		function onPointerStart(e) {
			...
			var newE = buildEvent(data.scale ? Events.scaleStart : Events.panStart, e, data.startX, data.startY,
				data.scaleD, 0, 0, "", "");
		...
		function onPointerMove(e) {
			...
			if (!data.passedAxis && data.axis && ((data.axisX && movedY) || (data.axisY && movedX))) {
				...
				if (!data.passedAxis &&
				    (!data.axis || (data.axis && (data.axisX && movedX) || (data.axisY && movedY)))) {
				...
				var fire = true,
					newE = buildEvent(data.scale ? Events.scale : Events.pan, e, newX, newY, data.scaleD, deltaX,
						deltaY, dirX, dirY);
