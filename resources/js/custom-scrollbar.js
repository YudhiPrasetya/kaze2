...
(function (factory) {
		if (typeof define === "function" && define.amd) {
			define(["jquery","./custom-scrollbar-core","./custom-scrollbar-touch"], factory);
		}
		else {
		...
		function construct(data) {
			...
			html +=
				'<button type="button" class="' + RawClasses.handle + '" aria-hidden="true" tabindex="-1"></button>';
			html += '</div></div>';

			data.paddingRight = parseInt(this.css("padding-right"), 10);
			data.paddingBottom = parseInt(this.css("padding-bottom"), 10);
			data.thisClasses =
				[RawClasses.base, data.theme, data.customClass, (data.horizontal ? RawClasses.horizontal : "")];
		...
		function onPanStart(e) {
			...
			if (data.horizontal) {
				handlePosition =
					data.handleLeft = e.pageX - offset.left /* + $Window.scrollLeft() */ - (data.handleWidth / 2);
			}
			else {
				handlePosition =
					data.handleTop = e.pageY - offset.top /* + $Window.scrollTop() */ - (data.handleHeight / 2);
			}
