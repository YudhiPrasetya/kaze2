...
class Item {
	...
	public function checkActivationStatus() {
		...
		if (true == $this->builder->conf['restful']) {
			...
			if ($this->builder->conf['rest_base']) {
				$base =
					(is_array($this->builder->conf['rest_base'])) ? implode('|', $this->builder->conf['rest_base']) :
	...
	public function active($pattern = null) {
		if (!is_null($pattern)) {
			...
			if (preg_match("@^{$pattern}\z@", Request::path())) $this->activate();
	 ...
	 * @return Item
	 */
	public function raw(string $title, array $options = array()) {
		$options['parent'] = $this->id;
	...
	public function divide(array $attributes = array()) {
		$attributes['class'] = Builder::formatGroupClass($attributes, array('class' => 'divider test'));
