 ...
 * Class Builder
 *
 * @method Collection whereNickname(mixed $nickname)
 * @method Collection whereId($id)
 * @method Collection whereParent($parent)
 * @method Collection whereActive(bool $active)
 * @method bool hasProperty(string $prop)
 *
...
class Builder {
	 ...
	 * @return string
	 */
	protected static function attributeElement(string $key, $value) {
		if (is_numeric($key)) {
			$key = $value;
		}
		if (!is_null($value)) {
	 ...
	 * @return Item
	 */
	public function raw(string $title, array $options = []) {
		$options['raw'] = true;
	 ...
	 * @return Item|null
	 */
	public function item(string $title) {
		return $this->whereNickname($title)->first();
	}
	 ...
	 * @return array
	 */
	protected static function mergeGroup(array $new, array $old) {
	 ...
	 * @return string
	 */
	public static function formatGroupPrefix(array $new, array $old) {
		if (isset($new['prefix'])) {
	 ...
	 * @return bool
	 */
	public static function isAbs(string $url) {
	...
	public function sortBy($sort_by, $sort_type = 'asc') {
		...
		$this->items = $this->items->sort(
			function ($f, $s) use ($sort_by, $sort_type) {
				$f = $f->$sort_by;
				$s = $s->$sort_by;
	 ...
	 * @return Builder
	 */
	public function spawn(array $name, Collection $collection) {
	 ...
	 * @return string
	 */
	public function asUl(array $attributes = [], array $children_attributes = [], array $item_attributes = [],
		callable $item_after_calback = null, array $item_after_calback_params = []) {
		return '<ul' . self::attributes($attributes) . '>' . $this->render(
				'ul',
				null,
				$children_attributes,
				$item_attributes,
	 ...
	 * @return string
	 */
	public function render(string $type = 'ul', int $parent = null, array $children_attributes = [],
		array $item_attributes = [], callable $item_after_calback = null, array $item_after_calback_params = []) {
		$items = '';
		...
		foreach ($this->whereParent($parent) as $item) {
			...
			if ($item->link) {
				$items .= $item->beforeHTML . '<a' . self::attributes($link_attr) .
				          (!empty($item->url()) ? ' href="' . $item->url() . '"' : '') . '>' . $item->title . '</a>' .
				          $item->afterHTML;
			}
			else {
				$items .= $item->title;
	 ...
	 * @return string
	 */
	public function asOl(array $attributes = [], array $children_attributes = [], array $item_attributes = [],
		callable $item_after_calback = null, array $item_after_calback_params = []) {
		return '<ol' . self::attributes($attributes) . '>' . $this->render(
				'ol',
				null,
				$children_attributes,
				$item_attributes,
	 ...
	 * @return string
	 */
	public function asDiv(array $attributes = [], array $children_attributes = [], array $item_attributes = [],
		callable $item_after_calback = null, array $item_after_calback_params = []) {
		return '<div' . self::attributes($attributes) . '>' . $this->render(
				'div',
				null,
				$children_attributes,
				$item_attributes,
	...
	public function __call($method, $args) {
		...
		return $this->items->filter(
			function ($item) use ($attribute, $value) {
	 ...
	 * @return Collection
	 */
	public function filterRecursive(string $attribute, $value) {
