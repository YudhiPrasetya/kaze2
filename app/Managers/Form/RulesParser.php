...
class RulesParser {
	 ...
	 * @return array
	 */
	protected function getRulesAsArray($rules): array {
	 ...
	 * @return array
	 */
	protected function parseStringRule($rules): array {
		$parameters = [];
	 ...
	 * @return array
	 */
	protected function parseParameters($rule, $parameter): array {
	 ...
	 */
	protected function digits($param): array {
		$digits = $param[0];
	 ...
	 */
	protected function digitsBetween($param): array {
		list($min, $max) = $param;
	 ...
	 */
	protected function min($param): array {
		$min = $param[0];
	 ...
	 */
	protected function max($param): array {
		$max = $param[0];
	 ...
	 */
	protected function between($param): array {
		list($min, $max) = $param;
	 ...
	 */
	protected function size($param): array {
		$size = $param[0];
	 ...
	 */
	protected function in($params): array {
		return [
	 ...
	 */
	protected function notIn($params): array {
		return [
			'pattern' => '(?:(?!^' . implode('$|^', $params) . '$).)*',
	 ...
	 */
	protected function after($params): array {
	 ...
	 */
	protected function before($params): array {
		if ($date = $this->getDateAttribute($params[0])) {
			return ['max' => $date];
		}
	 ...
	 * @see https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input#attr-accept
	 */
	protected function mimes($params): array {
