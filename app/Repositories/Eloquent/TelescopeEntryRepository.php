<?php
/**
 * This file is part of the Laravel project.
 *
 * Copyright (c) 2020 Eki Prathama Ramdhani <eq.petrucci@gmail.com>
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 *
 * @author Eki Prathama Ramdhani <eq.petrucci@gmail.com>
 * @file   TelescopeEntryRepository
 * @date   03/09/2020 09:02
 */

namespace App\Repositories\Eloquent;

use App\Models\TelescopeEntry;
use App\Repositories\TelescopeEntryRepositoryInterface;
use App\Repositories\Eloquent\RepositoryBase;


/**
 * Class TelescopeEntryRepository
 *
 * @package App\Repositories\Eloquent\Master
 */
class TelescopeEntryRepository extends RepositoryBase implements TelescopeEntryRepositoryInterface {
	public function __construct(TelescopeEntry $model) {
		parent::__construct($model);
	}
}
