...
class DistrictViewModel extends ViewModelBase {
	public function __construct(EloquentRepositoryInterface $repository, ?FormBuilder $formBuilder = null) {
		...
		$this->routeBasename = 'bank';
		$this->routeKey = 'bank';
		// $this->form = $this->formBuilder->create(BankForm::class);
	}

	public function select2SearchByCity(Request $request, City $city) {
		$search = $request->get('search', null);
		$results = collect([]);
		$items = null;

		if (!empty($search)) {
			$items = District::search($search)->where('city_id', $city->id);
		}
		else {
