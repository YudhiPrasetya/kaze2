 ...
 * @file   DataTransferObjectError.php
 * @date   29/08/2020 04.22
 */
...
class DataTransferObjectError extends \TypeError {
	...
	public static function invalidType(string $class, string $field, array $expectedTypes,
		$value): DataTransferObjectError {
		...
		return new self(
			"Invalid type: expected `{$class}::{$field}` to be of type `{$expectedTypes}`, instead got value `{$value}`, which is {$currentType}."
		);
	}
