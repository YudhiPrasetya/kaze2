...
class MySQLEngine extends Engine {
	...
	protected function shouldNotRun($builder) {
		return strlen($builder->query) < config('scout-driver.mysql.min_search_length');
	}

	protected function shouldUseFallback($builder) {
		return $this->mode->isFullText() &&
		       strlen($builder->query) < config('scout-driver.mysql.min_fulltext_search_length');
	}
